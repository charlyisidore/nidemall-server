...
CREATE TABLE `nidemall_comment` (
  ...
  `pic_urls` varchar(1023), -- 图片地址列表，采用JSON数组格式
...
CREATE TABLE `nidemall_coupon` (
  ...
  `start_time` datetime, -- 使用券开始时间
  `end_time` datetime, -- 使用券截至时间
  `add_time` datetime, -- 创建时间
  `update_time` datetime, -- 更新时间
  `deleted` tinyint(1) DEFAULT 0 -- 逻辑删除
); -- 优惠券信息及规则表
...
CREATE TABLE `nidemall_goods` (
  ...
  `gallery` varchar(1023), -- 商品宣传图片列表，采用JSON数组格式
  ...
  `pic_url` varchar(255), -- 商品页面商品图片
  `share_url` varchar(255), -- 商品分享海报
...
CREATE TABLE `nidemall_groupon` (
  ...
  `user_id` int(11) NOT NULL, -- 用户ID
  `share_url` varchar(255), -- 团购分享图片地址
...
CREATE TABLE `nidemall_log` (
  ...
  `result` varchar(127), -- 操作结果，或者成功消息，或者失败消息
...
CREATE TABLE `nidemall_notice` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `title` varchar(63), -- 通知标题
  `content` varchar(511), -- 通知内容
...
CREATE TABLE `nidemall_notice_admin` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `notice_id` int(11), -- 通知ID
  `notice_title` varchar(63), -- 通知标题
  `admin_id` int(11), -- 接收通知的管理员ID
  `read_time` datetime, -- 阅读时间，如果是NULL则是未读状态
  `add_time` datetime, -- 创建时间
  `update_time` datetime, -- 更新时间
  `deleted` tinyint(1) DEFAULT 0 -- 逻辑删除
); -- 通知管理员表
...
CREATE TABLE `nidemall_order` (
  ...
  `pay_id` varchar(63), -- 微信付款编号
  `pay_time` datetime, -- 微信付款时间
  `ship_sn` varchar(63), -- 发货编号
  `ship_channel` varchar(63), -- 发货快递公司
  `ship_time` datetime, -- 发货开始时间
  `refund_amount` decimal(10,2), -- 实际退款金额，（有可能退款金额小于实际支付金额）
  `refund_type` varchar(63), -- 退款方式
  `refund_content` varchar(127), -- 退款备注
  `refund_time` datetime, -- 退款时间
  `confirm_time` datetime, -- 用户确认收货时间
  `comments` smallint(6) DEFAULT 0, -- 待评价订单商品数量
  `end_time` datetime, -- 订单关闭时间
  `add_time` datetime, -- 创建时间
  `update_time` datetime, -- 更新时间
  `deleted` tinyint(1) DEFAULT 0 -- 逻辑删除
); -- 订单表
