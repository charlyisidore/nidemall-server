...
CREATE TABLE `nidemall_ad` (
  ...
  `startTime` datetime DEFAULT NULL, -- 广告开始时间
  `endTime` datetime DEFAULT NULL, -- 广告结束时间
  `enabled` tinyint(1) DEFAULT '0', -- 是否启动
  `addTime` datetime DEFAULT NULL, -- 创建时间
...
CREATE TABLE `nidemall_address` (
  ...
  `userId` int(11) NOT NULL DEFAULT '0', -- 用户表的用户ID
  ...
  `isDefault` tinyint(1) NOT NULL DEFAULT '0', -- 是否默认地址
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 收货地址表
...
CREATE TABLE `nidemall_admin` (
  ...
  `lastLoginTime` datetime DEFAULT NULL, -- 最近一次登录时间
...
CREATE TABLE `nidemall_aftersale` (
  ...
  `handleTime` datetime DEFAULT NULL, -- 管理员操作时间
...
CREATE TABLE `nidemall_cart` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `userId` int(11) DEFAULT NULL, -- 用户表的用户ID
  `goodsId` int(11) DEFAULT NULL, -- 商品表的商品ID
  `goodsSn` varchar(63) DEFAULT NULL, -- 商品编号
  `goodsName` varchar(127) DEFAULT NULL, -- 商品名称
  `productId` int(11) DEFAULT NULL, -- 商品货品表的货品ID
...
CREATE TABLE `nidemall_category` (
  ...
  `sortOrder` tinyint(3) DEFAULT '50', -- 排序
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 类目表
...
CREATE TABLE `nidemall_collect` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `userId` int(11) NOT NULL DEFAULT '0', -- 用户表的用户ID
  `valueId` int(11) NOT NULL DEFAULT '0', -- 如果type=0，则是商品ID；如果type=1，则是专题ID
...
CREATE TABLE `nidemall_comment` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `valueId` int(11) NOT NULL DEFAULT '0', -- 如果type=0，则是商品评论；如果是type=1，则是专题评论。
  ...
  `userId` int(11) NOT NULL DEFAULT '0', -- 用户表的用户ID
  `hasPicture` tinyint(1) DEFAULT '0', -- 是否含有图片
...
CREATE TABLE `nidemall_coupon` (
  ...
  `goodsType` smallint(6) DEFAULT '0', -- 商品限制类型，如果0则全商品，如果是1则是类目限制，如果是2则是商品限制。
  `goodsValue` varchar(1023) DEFAULT '[]', -- 商品限制值，goodsType如果是0则空集合，如果是1则是类目集合，如果是2则是商品集合。
  `code` varchar(63) DEFAULT NULL, -- 优惠券兑换码
  `timeType` smallint(6) DEFAULT '0', -- 有效时间限制，如果是0，则基于领取时间的有效天数days；如果是1，则startTime和endTime是优惠券有效期；
  `days` smallint(6) DEFAULT '0', -- 基于领取时间的有效天数days。
  `startTime` datetime DEFAULT NULL, -- 使用券开始时间
  `endTime` datetime DEFAULT NULL, -- 使用券截至时间
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 优惠券信息及规则表
...
CREATE TABLE `nidemall_coupon_user` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `userId` int(11) NOT NULL, -- 用户ID
  `couponId` int(11) NOT NULL, -- 优惠券ID
  `status` smallint(6) DEFAULT '0', -- 使用状态, 如果是0则未使用；如果是1则已使用；如果是2则已过期；如果是3则已经下架；
  `usedTime` datetime DEFAULT NULL, -- 使用时间
  `startTime` datetime DEFAULT NULL, -- 有效期开始时间
  `endTime` datetime DEFAULT NULL, -- 有效期截至时间
...
CREATE TABLE `nidemall_footprint` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `userId` int(11) NOT NULL DEFAULT '0', -- 用户表的用户ID
  `goodsId` int(11) NOT NULL DEFAULT '0', -- 浏览商品ID
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 用户浏览足迹表
...
CREATE TABLE `nidemall_goods` (
  ...
  `categoryId` int(11) DEFAULT '0', -- 商品所属类目ID
  ...
  `isOnSale` tinyint(1) DEFAULT '1', -- 是否上架
  `sortOrder` smallint(4) DEFAULT '100',
  `picUrl` varchar(255) DEFAULT NULL, -- 商品页面商品图片
  `shareUrl` varchar(255) DEFAULT NULL, -- 商品分享海报
  `isNew` tinyint(1) DEFAULT '0', -- 是否新品首发，如果设置则可以在新品首发页面展示
  `isHot` tinyint(1) DEFAULT '0', -- 是否人气推荐，如果设置则可以在人气推荐页面展示
...
CREATE TABLE `nidemall_goods_attribute` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `goodsId` int(11) NOT NULL DEFAULT '0', -- 商品表的商品ID
...
CREATE TABLE `nidemall_groupon` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `orderId` int(11) NOT NULL, -- 关联的订单ID
  `grouponId` int(11) DEFAULT '0', -- 如果是开团用户，则grouponId是0；如果是参团用户，则grouponId是团购活动ID
  `rulesId` int(11) NOT NULL, -- 团购规则ID，关联nidemall_groupon_rules表ID字段
  `userId` int(11) NOT NULL, -- 用户ID
  `shareUrl` varchar(255) DEFAULT NULL, -- 团购分享图片地址
  `creatorUserId` int(11) NOT NULL, -- 开团用户ID
  `creatorUserTime` datetime DEFAULT NULL, -- 开团时间
...
CREATE TABLE `nidemall_groupon_rules` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `goodsId` int(11) NOT NULL, -- 商品表的商品ID
  `goodsName` varchar(127) NOT NULL, -- 商品名称
  `picUrl` varchar(255) DEFAULT NULL, -- 商品图片或者商品货品图片
  `discount` decimal(63,0) NOT NULL, -- 优惠金额
  `discountMember` int(11) NOT NULL, -- 达到优惠条件的人数
  `expireTime` datetime DEFAULT NULL, -- 团购过期时间
...
CREATE TABLE `nidemall_keyword` (
  ...
  `isHot` tinyint(1) NOT NULL DEFAULT '0', -- 是否是热门关键字
  `isDefault` tinyint(1) NOT NULL DEFAULT '0', -- 是否是默认关键字
  `sortOrder` int(11) NOT NULL DEFAULT '100', -- 排序
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 关键字表
...
CREATE TABLE `nidemall_notice` (
  ...
  `adminId` int(11) DEFAULT '0', -- 创建通知的管理员ID，如果是系统内置通知则是0.
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 通知表
...
CREATE TABLE `nidemall_notice_admin` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `noticeId` int(11) DEFAULT NULL, -- 通知ID
  `noticeTitle` varchar(63) DEFAULT NULL, -- 通知标题
  `adminId` int(11) DEFAULT NULL, -- 接收通知的管理员ID
  `readTime` datetime DEFAULT NULL, -- 阅读时间，如果是NULL则是未读状态
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 通知管理员表
...
CREATE TABLE `nidemall_order` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `userId` int(11) NOT NULL, -- 用户表的用户ID
  `orderSn` varchar(63) NOT NULL, -- 订单编号
  `orderStatus` smallint(6) NOT NULL, -- 订单状态
  `aftersaleStatus` smallint(6) DEFAULT '0', -- 售后状态，0是可申请，1是用户已申请，2是管理员审核通过，3是管理员退款成功，4是管理员审核拒绝，5是用户已取消
  ...
  `refundTime` datetime DEFAULT NULL, -- 退款时间
  `confirmTime` datetime DEFAULT NULL, -- 用户确认收货时间
  `comments` smallint(6) DEFAULT '0', -- 待评价订单商品数量
  `endTime` datetime DEFAULT NULL, -- 订单关闭时间
  `addTime` datetime DEFAULT NULL, -- 创建时间
  `updateTime` datetime DEFAULT NULL, -- 更新时间
  `deleted` tinyint(1) DEFAULT '0' -- 逻辑删除
); -- 订单表
...
CREATE TABLE `nidemall_order_goods` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `orderId` int(11) NOT NULL DEFAULT '0', -- 订单表的订单ID
  `goodsId` int(11) NOT NULL DEFAULT '0', -- 商品表的商品ID
  `goodsName` varchar(127) NOT NULL DEFAULT '', -- 商品名称
  `goodsSn` varchar(63) NOT NULL DEFAULT '', -- 商品编号
  `productId` int(11) NOT NULL DEFAULT '0', -- 商品货品表的货品ID
...
CREATE TABLE `nidemall_permission` (
  `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT,
  `roleId` int(11) DEFAULT NULL, -- 角色ID
...
CREATE TABLE `nidemall_topic` (
  ...
  `sortOrder` int(11) DEFAULT '100', -- 排序
...
CREATE TABLE `nidemall_user` (
  ...
  `userLevel` tinyint(3) DEFAULT '0', -- 0 普通用户，1 VIP用户，2 高级VIP用户
